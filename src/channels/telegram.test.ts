...
// Mock @grammyjs/stream
const mockStreamMessage = vi.fn().mockResolvedValue([]);
vi.mock('@grammyjs/stream', () => ({
  streamApi: vi.fn(() => ({ streamMessage: mockStreamMessage })),
}));
...
async function triggerCallbackQuery(ctx: ReturnType<typeof createCallbackCtx>) {
...
describe('TelegramChannel', () => {
  ...
  describe('connection lifecycle', () => {
    ...
    it('registers command and message handlers on connect', async () => {
      ...
      expect(currentBot().filterHandlers.has('callback_query:data')).toBe(true);
  ...
  describe('callback queries', () => {
    it('stores topic callback queries under the topic JID and acknowledges them', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      await triggerCallbackQuery(ctx);

      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300:topic:77',
        expect.objectContaining({
          id: 'cb-1',
          chat_jid: 'tg:100200300:topic:77',
          content: '@Andy [Button: approve]',
        }),
      );
  ...
  describe('sendMessage', () => {
    ...
    it('uses streamMessage for long messages (plugin handles splitting)', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      // Long messages go through streamMessage, not bot.api.sendMessage
      expect(mockStreamMessage).toHaveBeenCalledTimes(1);
      expect(mockStreamMessage).toHaveBeenCalledWith(
        100200300,
        expect.any(Number),
        expect.anything(), // async generator
        undefined,
        undefined,
      );
    ...
    it('uses streamMessage for messages above threshold', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('includes message_thread_id and inline keyboard for topic replies', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:100200300:topic:77', 'Choose', {
    ...
    it('preserves message_thread_id and inline keyboard for streamed topic replies', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:100200300:topic:77', 'x'.repeat(200), {
        buttons: [
    ...
    it('serializes multi-row keyboards without trailing empty rows', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:100200300', 'Choose', {
        buttons: [
          [
            { text: 'One', data: 'one' },
            { text: 'Two', data: 'two' },
    ...
    it('skips invalid keyboards with empty rows', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('skips invalid keyboards with empty button text', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:100200300', 'Choose', {
        buttons: [[{ text: '', data: 'approve' }]],
      });
    ...
    it('skips invalid keyboards with callback data over 64 bytes', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.sendMessage('tg:100200300', 'Choose', {
        buttons: [[{ text: 'Too long', data: '😀'.repeat(17) }]],
      });
  ...
  describe('live messages', () => {
    it('sends a live message and returns its message id', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      currentBot().api.sendMessage.mockResolvedValueOnce({ message_id: 777 });

      const messageId = await channel.sendLiveMessage('tg:100200300', 'Working');
    ...
    it('sends live messages into topics', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      currentBot().api.sendMessage.mockResolvedValueOnce({ message_id: 778 });

      await channel.sendLiveMessage('tg:100200300:topic:77', 'Working');
    ...
    it('edits live messages', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.editLiveMessage('tg:100200300', 42, 'Updated');
    ...
    it('ignores message is not modified errors when editing live messages', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      await expect(
        channel.editLiveMessage('tg:100200300', 42, 'Updated'),
      ).resolves.toBeUndefined();
    });

    it('deletes live messages', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.deleteLiveMessage('tg:100200300', 42);
  ...
  describe('setTyping', () => {
    ...
    it('sends typing action to the topic thread when present', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      await channel.setTyping('tg:100200300:topic:77', true);
